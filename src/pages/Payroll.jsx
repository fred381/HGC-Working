...
export default function Payroll() {
  ...
  function setAdj(carerId, field, value) {
    setAdjustments(prev => ({
      ...prev,
      [carerId]: { ...prev[carerId], [field]: value },
    }))
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-hgc-900">Payroll</h2>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-hgc-600 text-white px-4 py-2.5 rounded-lg hover:bg-hgc-700 transition-all duration-200 text-sm font-medium shadow-sm hover:shadow disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-sm"
        >
      ...
      {activeCarers.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-16 text-center">
          <div className="w-14 h-14 rounded-full bg-hgc-50 flex items-center justify-center mx-auto mb-4">
            <FileSpreadsheet size={28} className="text-hgc-400" />
          </div>
          <h3 className="text-base font-semibold text-hgc-900 mb-1">No active carers</h3>
          <p className="text-sm text-gray-500 max-w-sm mx-auto">
            Add carers on the Carers page to generate payroll. Only active carers appear here.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full" style={{ minWidth: '960px' }}>
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wider sticky left-0 bg-gray-50 z-10 min-w-[180px]">Carer</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {payrollRows.map(row => {
                  ...
                  return (
                    <tr key={id} className="hover:bg-gray-50/60 transition-colors duration-150">
                      <td className="px-4 py-3 sticky left-0 bg-white z-10">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-hgc-900 truncate">{row.carer.name}</p>
                            <p className="text-[11px] text-gray-400 font-mono">{row.carer.employeeId || '—'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-3 text-center">
                        <span className={`text-sm font-semibold ${row.daysWorked > 0 ? 'text-hgc-900' : 'text-gray-400'}`}>
                          {row.daysWorked}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-center">
              ...
              <tfoot>
                <tr className="bg-gray-50 border-t-2 border-gray-200">
                  <td className="px-4 py-3 sticky left-0 bg-gray-50 z-10">
                    <span className="text-sm font-bold text-hgc-900">Totals</span>
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        placeholder="0.00"
        className="w-full pl-6 pr-2 py-1.5 text-sm text-right border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-hgc-600 focus:border-transparent outline-none placeholder:text-gray-300 transition-shadow duration-200"
      />
    </div>
  )
}
